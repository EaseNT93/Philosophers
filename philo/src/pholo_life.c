...
/*   Updated: 2021/12/15 16:58:00 by dpants           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*philo_life(void *philo)
{
	...
	while (!cfg->die_f)
	{
		...
		if (cfg->philo_count == 1 || cfg->eat_count_f == 2)
			break ;
		print_msg(cfg, philo->nb, "is sleeping");
		philo_w8(cfg, get_time(), cfg->sleep_t);
		print_msg(cfg, philo->nb, "is thinking");
	}
	return (NULL);
}
...
int	start_life_cycle(t_cfg *cfg)
{
	...
	while (cfg->philo_count > i)
	{
		if (pthread_create(&(philo[i].thread_id), NULL, philo_life, &(philo[i])))
			return (1);
	...
	i = 0;
	while (cfg->philo_count > i)
		pthread_join(philo[i++].tread_id, NULL); 
	i = 0;
	while (cfg->philo_count > i)
